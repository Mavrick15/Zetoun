...
const pathContentMap = {
  ...
  '/formations/windows-administration': {
    ...
    text: "Contactez notre équipe des formatuers dès aujourd'hui pour obtenir plus d'informations sur les dates de formation, les tarifs et les modalités d'inscription. Nous sommes là pour vous accompagner.",
  },
...
const ContactInfoPage = ({ dynamicTitle, dynamicText }) => {
  const location = useLocation();

  const defaultTitle = "Une question n'hésitez pas à nous contacter !";
  const defaultText = "Nous sommes là pour vous aider à chaque étape. Que ce soit pour un projet, une consultation ou simplement pour discuter de vos besoins, notre équipe est à votre écoute.";

  const content = pathContentMap[location.pathname] || {
    title: defaultTitle,
    text: defaultText,
  };
  ...
  if (location.pathname === '/add/contact-nous' ) {
    return null;
  }
