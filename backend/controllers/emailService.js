...
  CLIENT_CONFIRMATION_ERROR: (email) => `Erreur lors de l'envoi de l'e-mail de confirmation au client ${email}:`,
  VERIFICATION_EMAIL_SUBJECT: 'Vérifiez votre adresse email pour Zetoun Labs',
  VERIFICATION_EMAIL_INFO: (email) => `Email de vérification envoyé à ${email}`,
  VERIFICATION_EMAIL_ERROR: (email) => `Erreur lors de l'envoi de l'e-mail de vérification à ${email}:`,
};
...
    from: process.env.EMAIL_FROM,
    to: email,
    subject: EMAIL_MESSAGES.VERIFICATION_EMAIL_SUBJECT,
    html: `
        <div style="${HTML_STYLES.CLIENT_MAIN_DIV}">
            <div style="${HTML_STYLES.CLIENT_HEADER_DIV}">
                <h1 style="${HTML_STYLES.CLIENT_H1_TITLE}">ZETOUN LABS</h1>
...
            </div>
            <div style="${HTML_STYLES.CLIENT_CONTENT_DIV}">
                <p style="${HTML_STYLES.CLIENT_P_REGULAR}">Bonjour <strong style="${HTML_STYLES.CLIENT_P_BOLD_TEXT}">${name}</strong>,</p>
                <p>Merci de vous être inscrit sur Zetoun Labs ! Pour activer votre compte et commencer à explorer nos formations, veuillez vérifier votre adresse email en cliquant sur le lien ci-dessous :</p>
                <p style="text-align: center; margin: 30px 0;">
                    <a href="${verificationLink}" style="display: inline-block; padding: 12px 25px; background-color: #000; color: #fff; text-decoration: none; border-radius: 5px; font-size: 16px; font-weight: bold;">
                        Vérifier mon email
                    </a>
                </p>
                <p>Ce lien est valable pendant 1 heure. Si vous n'avez pas créé de compte chez nous, veuillez ignorer cet email.</p>
                <p style="${HTML_STYLES.CLIENT_P_MARGIN_TOP} ${HTML_STYLES.CLIENT_P_SMALL_FONT}">
                    Cordialement,<br/>
                    L'équipe de Zetoun Labs<br/>
                    <a href="${process.env.FRONTEND_URL}" style="color: #000; text-decoration: none;">www.zetounlabs.cd</a>
...
    logger.info(EMAIL_MESSAGES.VERIFICATION_EMAIL_INFO(email));
  } catch (error) {
    logger.error(EMAIL_MESSAGES.VERIFICATION_EMAIL_ERROR(email), { message: error.message, stack: error.stack });
    throw error;
  }
};
